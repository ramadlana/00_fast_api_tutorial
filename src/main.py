...
async def login_http_only(auth_details: schemas.AuthDetailsRequest, response: Response):
    ...
    if (user is None) or (not auth_handler.verify_password(auth_details.password, user['password'])):
        raise HTTPException(status_code=401, detail='Invalid username and/or password')
    token = auth_handler.encode_token(user['username'])
    response.set_cookie(key='token', value=token, httponly=True,domain="https://3545-115-178-194-222.ap.ngrok.io", max_age=18000, expires=18000, samesite='none',secure=True)
